// Laba6(base).cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
struct RK4Table {
    ...
    static constexpr std::array<std::array<double, stages>, stages> table = { { {0,0,0,0}, {0.5,0,0,0},{0,0.5, 0},{0,0,1,0} } };
...
public:
    ...
    std::array<StateAndArg, N> integrateDeq(
        ...
        for (unsigned int j = 0; j < N - 1; j++) {
            ...
            Res[j + 1].state = Res[j].state + step * (K[0] * A.bString[0] + K[1] * A.bString[1] + K[2] * A.bString[2] + K[3] * A.bString[3]);
        }
        return Res;
    };
...
public:
    ...
    std::array<StateAndArg, N> integrateSyst(
        ...
        for (unsigned int j = 0; j < N - 1; j++) {
            ...
            // находим решение в следующей точке
            Res[j + 1].arg = Res[j].arg + step;
            Res[j + 1].state[0] = Res[j].state[0] + step * (K[0] * A.bString[0] + K[1] * A.bString[1] + K[2] * A.bString[2] + K[3] * A.bString[3]);
            Res[j + 1].state[1] = Res[j].state[1] + step * (M[0] * A.bString[0] + M[1] * A.bString[1] + M[2] * A.bString[2] + M[3] * A.bString[3]);
        }
        return Res;
    };
    //считает ошибку и печатает логарифм шага и логарифм ошибки, тангенс угла наклона и решение для шага h=1/128 
